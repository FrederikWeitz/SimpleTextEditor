...
public class StageListener implements ApplicationListener<StageReadyEvent> {
    ...
    public static class FoldableStyledArea extends GenericStyledArea<ParStyle, Either<String, LinkedImage>, TextStyle> {
        ...
        public void unfoldText(int startingFromPos) {
            startingFromPos = offsetToPosition(startingFromPos, Backward).getMajor();
    ...
    public void onApplicationEvent(StageReadyEvent event) {
        ...
        area.beingUpdatedProperty().addListener((o, old, beingUpdated) -> {
            if(!beingUpdated) {
                ...
                if(selection.getLength() != 0) {
                    ...
                    Color[] backgrounds = styles.styleStream().map(s -> s.backgroundColor.orElse(null)).distinct().toArray(Color[]::new);
        ...
        scene.getStylesheets().add("css/rich-text.css");
        mainStage.setScene(scene);
        area.requestFocus();
        mainStage.setTitle("Rich Text Demo");
        mainStage.show();
    }
